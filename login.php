...
<?php
 ...
 if (isset($_POST['email']) && isset($_POST['password']) ){   //check value insert
   ...
   $query = "SELECT * FROM `register` WHERE email='$email' AND password='$password'";   //retrieve all username n passwordf
   ...
   if($count == 1){
	 ...
	 $url="examiner.php?eid=".$row['eid'];
	 echo "$url";
	header('Location: '.$url);
	// $row=mysqli_fetch_assoc($results);
	// $url="examiner.php?id=".$row['eid'];
   }else{
